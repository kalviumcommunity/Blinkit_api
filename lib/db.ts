...
export async function initializeDatabase() {
  ...
  globals.blinkitSchemaReady ??= transaction(async (client) => {
    // Serialize first-run setup across workers. Old tables stay untouched in public.
    ...
    await client.query(`
      ...
      CREATE TABLE IF NOT EXISTS inventory_logs (
        ...
        manager_id INTEGER NOT NULL REFERENCES app_users(id),
        change INTEGER NOT NULL CHECK (change != 0),
        old_stock INTEGER NOT NULL CHECK (old_stock >= 0),
        new_stock INTEGER NOT NULL CHECK (new_stock >= 0),
        request_id UUID UNIQUE NOT NULL,
        created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
        CHECK (new_stock::bigint = old_stock::bigint + change::bigint)
      );
      CREATE INDEX IF NOT EXISTS logs_created ON inventory_logs(created_at DESC, id DESC);
      CREATE INDEX IF NOT EXISTS logs_product ON inventory_logs(product_id);
    `);
  }).catch((error) => {
